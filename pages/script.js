...
// 页面加载时尝试从KV获取最新的汇率数据
async function loadLatestRateFromKV() {
    try {
        const res = await fetch(`${CONFIG.WORKER_URL}/api/rate`);
        if (res.ok) {
            ...
            if (data.success && data.rate) {
                ...
                document.getElementById('rate-update-time').textContent = `KV缓存: ${formatDateTime(new Date(data.timestamp * 1000))}`;
                calculateProfit();
                return true;
            }
        }
    } catch (err) {
        console.log('从KV获取汇率失败，使用默认值:', err);
    }
    return false;
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (shareId) {
        ...
        // 否则尝试从KV获取最新汇率数据，如果没有则使用默认值
        loadLatestRateFromKV().then(success => {
            if (!success) {
                // KV获取失败，设置默认汇率
...
async function fetchExchangeRate() {
    ...
    try {
        ...
        if (data.success && data.rates?.CNY) {
            ...
            document.getElementById('exchange-rate').value = rate;
            document.getElementById('current-rate').textContent = rate;
            document.getElementById('rate-update-time').textContent = `实时获取: ${formatDateTime(new Date(data.timestamp * 1000))}`;
            calculateProfit();
        ...
        // 尝试从KV获取缓存的汇率数据
        const kvRes = await fetch(`${CONFIG.WORKER_URL}/api/rate`);
        if (kvRes.ok) {
            const kvData = await kvRes.json();
            if (kvData.success && kvData.rate) {
                const rate = parseFloat(kvData.rate).toFixed(4);
                document.getElementById('exchange-rate').value = rate;
                document.getElementById('current-rate').textContent = rate;
                document.getElementById('rate-update-time').textContent = `KV缓存: ${formatDateTime(new Date(kvData.timestamp * 1000))}`;
                showNotification('warning', '实时汇率获取失败', `使用缓存汇率: ${rate}`);
            } else {
                throw err; // KV也没有有效数据，抛出原始错误
            }
        } else {
            // KV也访问失败，使用默认值
            document.getElementById('exchange-rate').value = CONFIG.DEFAULT_RATE;
